#!/usr/bin/env Rscript
# bray_nmds_clean_plot.R
#
# Generate clean Bray-Curtis NMDS ordination plot without envfit overlays
...
# ---- 2.  Load pre-computed Bray-Curtis distance matrix --------------------
...
# ---- 3.  Run 4D NMDS on pre-computed Bray-Curtis --------------------------

cat("Running 4D NMDS on", nrow(bray_matrix_subset), "samples using QIIME2 Bray-Curtis distances …\n")

set.seed(123)
nmds_result <- metaMDS(bray_matrix_subset, k = 4, trymax = 100, trace = FALSE)
...
cat("Saving Bray-Curtis NMDS plot...\n")

# Save PNG version
ggsave(
  filename = "BetaAnalysis/nmds_plot_bray.png",
  plot = p1,
  width = 10,
  height = 8,
  dpi = 300,
...
ggsave(
  filename = "BetaAnalysis/nmds_plot_bray.tiff",
  plot = p1,
  width = 10,
  height = 8,
  dpi = 600,
...
cat("Bray-Curtis NMDS plots saved:\n")
cat("  - BetaAnalysis/nmds_plot_bray.png\n")
cat("  - BetaAnalysis/nmds_plot_bray.tiff\n")
